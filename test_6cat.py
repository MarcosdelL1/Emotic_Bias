...
from emotic_dataset_6cat import Emotic_PreDataset_6cat
...
def test_scikit_ap(cat_preds, cat_labels, ind2cat):
  ...
  '''
  num_categories = len(ind2cat)
  ap = np.zeros(num_categories, dtype=np.float32)
...
def get_thresholds(cat_preds, cat_labels):
  ...
  '''
  num_categories = cat_preds.shape[0]
  thresholds = np.zeros(num_categories, dtype=np.float32)
  for i in range(num_categories):
...
def test_data(models, device, data_loader, ind2cat, ind2vad, num_images, result_dir='./', test_type='val'):
    ...
    num_categories = len(ind2cat)
    cat_preds = np.zeros((num_images, num_categories))
    cat_labels = np.zeros((num_images, num_categories))
    ...
    scipy.io.savemat(os.path.join(result_dir, '%s_cat_preds_6cat.mat' %(test_type)), mdict={'cat_preds':cat_preds})
    scipy.io.savemat(os.path.join(result_dir, '%s_cat_labels_6cat.mat' %(test_type)), mdict={'cat_labels':cat_labels})
    scipy.io.savemat(os.path.join(result_dir, '%s_cont_preds_6cat.mat' %(test_type)), mdict={'cont_preds':cont_preds})
    scipy.io.savemat(os.path.join(result_dir, '%s_cont_labels_6cat.mat' %(test_type)), mdict={'cont_labels':cont_labels})
    ...
    np.save(os.path.join(result_dir, '%s_thresholds_6cat.npy' %(test_type)), thresholds)
    print ('saved thresholds')


def test_emotic_6cat(result_path, model_path, ind2cat, ind2vad, context_norm, body_norm, args):
    ''' Prepare test data and test models on the same for 6-category emotion classification.
    ...
    '''    
    # Prepare models 
    model_context = torch.load(os.path.join(model_path,'model_context_6cat.pth'))
    model_body = torch.load(os.path.join(model_path,'model_body_6cat.pth'))
    emotic_model = torch.load(os.path.join(model_path,'model_emotic_6cat.pth'))
    ...
    test_dataset = Emotic_PreDataset_6cat(test_context, test_body, test_cat, test_cont, test_transform, context_norm, body_norm)

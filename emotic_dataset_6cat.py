...
from emotion_mapping import convert_labels_26_to_6


class Emotic_PreDataset_6cat(Dataset):
  ''' Custom Emotic dataset class for 6-category emotion classification. Use preprocessed data stored in npy files. '''
  def __init__(self, x_context, x_body, y_cat, y_cont, transform, context_norm, body_norm):
    super(Emotic_PreDataset_6cat,self).__init__()
    self.x_context = x_context
    self.x_body = x_body
    # Convert 26-category labels to 6-category labels
    self.y_cat = convert_labels_26_to_6(y_cat)
    self.y_cont = y_cont
...
class Emotic_CSVDataset_6cat(Dataset):
  ''' Custom Emotic dataset class for 6-category emotion classification. Use csv files and generated data at runtime. '''
  def __init__(self, data_df, cat2ind, transform, context_norm, body_norm, data_src = './'):
    super(Emotic_CSVDataset_6cat,self).__init__()
  ...
  def cat_to_one_hot(self, cat):
    one_hot_cat = np.zeros(6)  # 6 emotion categories
    for em in cat:

...
def infer(context_norm, body_norm, ind2cat, ind2vad, device, thresholds, models, image_context_path=None, image_context=None, image_body=None, bbox=None, to_print=True):
  ''' Perform inference over an image for 6-category emotion classification. 
  :param context_norm: List containing mean and std values for context images. 
  :param body_norm: List containing mean and std values for body images. 
  ...
  if to_print == True:
    print ('\n Image predictions (6-category system)')
...
def inference_emotic_6cat(images_list, model_path, result_path, context_norm, body_norm, ind2cat, ind2vad, args):
  ''' Infer on list of images defined in a text file for 6-category emotion classification. Save the results in inference_file.txt in the directory specified by the result_path. 
  ...
  thresholds = torch.FloatTensor(np.load(os.path.join(result_path, 'val_thresholds_6cat.npy'))).to(device) 
  model_context = torch.load(os.path.join(model_path,'model_context_6cat.pth')).to(device)
  model_body = torch.load(os.path.join(model_path,'model_body_6cat.pth')).to(device)
  emotic_model = torch.load(os.path.join(model_path,'model_emotic_6cat.pth')).to(device)
  model_context.eval()
  model_body.eval()
  emotic_model.eval()
  ...
  result_file = os.path.join(result_path, 'inference_list_6cat.txt')

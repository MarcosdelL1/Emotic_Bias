...
from train_6cat import train_emotic_6cat
from test_6cat import test_emotic_6cat
from inference_6cat import inference_emotic_6cat
from emotion_mapping import NEW_EMOTIONS
...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=6) # use batch size = number of categorical emotion classes (6)
    # Generate args
...
if __name__ == '__main__':
    ...
    print ('Using 6-category emotion classification system')

    result_path, model_path, train_log_path, val_log_path = check_paths(args)

    # Use the new 6-category emotion system
    cat = NEW_EMOTIONS
    cat2ind = {}
    ind2cat = {}
    ...
    print(f"6 Emotion Categories: {cat}")

    if args.mode == 'train':
        if args.data_path is None:
            raise ValueError('Data path not provided. Please pass a valid data path for training')
        with open(os.path.join(args.experiment_path, 'config_6cat.txt'), 'w') as f:
            print(args, file=f)
        train_emotic_6cat(result_path, model_path, train_log_path, val_log_path, ind2cat, ind2vad, context_norm, body_norm, args)
    elif args.mode == 'test':
        if args.data_path is None:
            raise ValueError('Data path not provided. Please pass a valid data path for testing')
        test_emotic_6cat(result_path, model_path, ind2cat, ind2vad, context_norm, body_norm, args)
        ...
        inference_emotic_6cat(args.inference_file, model_path, result_path, context_norm, body_norm, ind2cat, ind2vad, args)
    else:

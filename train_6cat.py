...
from test_6cat import test_data
...
def train_data(opt, scheduler, models, device, train_loader, val_loader, disc_loss, cont_loss, train_writer, val_writer, model_path, args):
    ...
    model_body.to("cpu")
    torch.save(emotic_model, os.path.join(model_path, 'model_emotic_6cat.pth'))
    torch.save(model_context, os.path.join(model_path, 'model_context_6cat.pth'))
    torch.save(model_body, os.path.join(model_path, 'model_body_6cat.pth'))
    print ('saved models')


def train_emotic_6cat(result_path, model_path, train_log_path, val_log_path, ind2cat, ind2vad, context_norm, body_norm, args):
    ''' Prepare dataset, dataloders, models for 6-category emotion classification. 
    ...
    train_dataset = Emotic_PreDataset_6cat(train_context, train_body, train_cat, train_cont, train_transform, context_norm, body_norm)
    val_dataset = Emotic_PreDataset_6cat(val_context, val_body, val_cat, val_cont, test_transform, context_norm, body_norm)
    ...
    emotic_model = Emotic_6cat(list(model_context.children())[-1].in_features, list(model_body.children())[-1].in_features)
